...
import { extend, debounce, getMousePos } from '../utils/Utils';

const is3DBuggy = navigator.userAgent.indexOf('Firefox') > 0;

export function PieceMaker(el, options) {
    this.el = el;
 ...
 */
 PieceMaker.prototype.options = {
    ...
    tilt: {maxRotationX: -2, maxRotationY: 3, 
        maxTranslationX: 6, maxTranslationY: -2}
};
    ...
    const self = this,
          ...
          onMouseMoveFn = function(ev) {
            requestAnimationFrame(function() {
                ...
                const mousepos = getMousePos(ev),
                      rotX = 2*self.options.tilt.maxRotationX/self.win.height
                      *mousepos.y - self.options.tilt.maxRotationX,
                      rotY = 2*self.options.tilt.maxRotationY/self.win.width
                      *mousepos.x - self.options.tilt.maxRotationY,
                      transX = 2*self.options.tilt.maxTranslationX/self.win.width
                      *mousepos.x - self.options.tilt.maxTranslationX,
                      transY = 2*self.options.tilt.maxTranslationY/self.win.height
                      *mousepos.y - self.options.tilt.maxTranslationY;

                self.el.style.transform = 
                'perspective(1000px) translate3d(' + transX + 'px,' 
                + transY + 'px,0) rotate3d(1,0,0,' + rotX 
                + 'deg) rotate3d(0,1,0,' + rotY + 'deg)';
            });
          },
          // Window resize.
          debounceResizeFn = debounce(function() {
            ...
            for(let i = 0, len = self.pieces.length; i < len; ++i) {
                const 
                      w = Math.round(self.dimensions.width
                        / self.options.pieces.columns),
                      h = Math.round(self.dimensions.height
                        / self.options.pieces.rows),
                      piece = self.pieces[i];
                
                piece.style.width = w + 'px';
                piece.style.height = h + 'px';
                piece.style.backgroundSize = w 
                * self.options.pieces.columns + 'px auto';
                self.el.style.width = w 
                * self.options.pieces.columns + 'px';
                self.el.style.height = h 
                * self.options.pieces.rows + 'px';
            }
          }, 10);
...
PieceMaker.prototype.animatePieces = function(dir, callback) {
    ...
    anime({
        ...
        //easing: dir === 'out' ? [0.2,1,0.3,1] : [0.8,1,0.3,1], #Issue 1
        easing: dir === 'out' ? 'easeOutElastic(2, 2)' : 'easeInOutElastic(2, 2)',
...
PieceMaker.prototype.fxCustom = function(dir) {
    ...
    anime({
        ...
        // easing: dir === 'left' ? [0.2,1,0.3,1] : [0.8,0,0.7,0], #Issue 1
        easing: dir === 'left' ? 'easeOutElastic(2, 2)' : 'easeInOutElastic(2, 2)',
        ...
        opacity: {
            ...
            // easing: dir === 'left' ? 'linear' : [0.8,0,0.7,0] #Issue 1
            easing: dir === 'left' ? 'linear' : 'linear'
        }
    });
};
...
PieceMaker.prototype.fxCustomReset = function(dir, callback) {
    ...
    anime({
        ...
        // easing: dir === 'left' ? [0.8,0,0.7,0] : [0.2,1,0.3,1], #Issue 1
        easing: dir === 'left' ? 'easeOutElastic(2, 2)' : 'easeInOutElastic(2, 2)',
        delay: function(t,i,c) {
        ...
        opacity: {
            ...
            // easing: dir === 'left' ? [0.8,0,0.7,0] : 'linear' #Issue 1
            easing: dir === 'left' ? 'linear' : 'linear'
        },
        complete: callback
    });
