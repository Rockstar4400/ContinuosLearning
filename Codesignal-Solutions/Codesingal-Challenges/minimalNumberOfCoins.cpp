...
CODEFIGHTS - BOTS - LEVEL 01 - MINIMAL NUMBER OF COINS
...
coins. coins[0] = 1, and for each i (0 < i < coins.length)
coins[i] is divisible by coins[i - 1].
Find the minimal number of banana-coins you'll have to
spend to buy a banana given the banana's price.
...
// Loop ends to the while loop ends
    while(price >= coins[i]){
// While price >= value of coins[i] then
// price = price - value of coins[i]
        price = price - coins[i];
// When price >= coins ends loop and increase result + 1
        result++;
    }
  }
...
cout << minimalNumberOfCoins({1, 5, 10, 100}, 239) << '\n'; // 10
return 0;
}
